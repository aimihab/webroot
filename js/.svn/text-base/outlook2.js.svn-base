...
function InitLeftMenu() {
	$.ajax({
             type: "POST",
             url: "webapi/admin_main_api.php?action=initmenu",
             data: {},
             dataType: "text",
             success: function(data){
			    ...
			    $.each(data.menus, function(i, n) {
			    	if(n.pid==1000){
			    		 var s=" data-options=iconCls:'"+n.icon+"'";
			    		 menulist += '<div title="'+n.menuname+'"'+s+' style="overflow:auto;">';
			    		 menulist += '<ul>';
			    		 $.each(data.menus, function(j, o) {
			    		 	// alert(n.id!=o.id);
			    		 	if(n.id!=o.id && o.pid==n.id){
			    ...
			    $('#accordion').append(menulist);       //d为ajax获取菜单html
				$('#accordion li a').click(function(){
   				...
   				$('#accordion').accordion({ border:false,fit:true });
             },
